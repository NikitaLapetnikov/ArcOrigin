...
async function deployFixture() {
  ...
  const Factory = await ethers.getContractFactory("ArcForgeFactoryV7");
...
async function launch(fixture) {
  ...
  return {
    token: await ethers.getContractAt("ArcForgeTokenV7", event.args.token),
...
async function predictNextToken(factory, creator, params) {
  ...
  const Token = await ethers.getContractFactory("ArcForgeTokenV7");
...
describe("ArcOrigin V7 direct Uniswap V3 launches", function () {
