...
export function CreatorDashboard({ address }: { address: string }) {
  ...
  const crossed = tokens.filter((token) => token.status === "Crossed").length;
  ...
  return <div className="container-shell pb-20">
    ...
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-3"><StatCard label="Indexed launches" value={String(tokens.length)} detail="Confirmed Factory events"/><StatCard label="Crossed" value={String(crossed)} detail="Confirmed market-cap status"/><StatCard label="Tracked volume" value={isPartial ? "—" : money(volume,true)} detail="Confirmed pool swaps"/></div>
