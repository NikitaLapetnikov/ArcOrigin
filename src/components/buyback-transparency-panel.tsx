...
export function BuybackTransparencyPanel({ token }: { token: TokenData }) {
  const enabled = token.automaticBuyback === true;
  const { snapshot, loading, error, refresh } = useBuybackSnapshot(token.address, enabled);
  ...
  const status = snapshot?.ready
    ...
    : snapshot && snapshot.reserveUsdc < 1
      ...
      : snapshot && snapshot.nextExecutionAt > now
        ...
        : { label: loading ? "Loading" : "Protected", tone: "neutral" as const, detail: "TWAP and price checks apply" };
  ...
  return <Panel className="overflow-hidden rounded-[24px] shadow-none">
    <div className="flex items-start justify-between gap-3 border-b border-line/70 px-4 py-4">
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2"><h2 className="text-sm font-semibold text-white">Automatic buybacks</h2><Badge tone={status.tone}>{status.label}</Badge></div>
    ...
    <div className="grid grid-cols-2 border-b border-line/70 sm:grid-cols-4 xl:grid-cols-2 2xl:grid-cols-4">
      <BuybackMetric label="USDC spent" value={snapshot ? money(snapshot.totalUsdcSpent, true) : "—"} />
      <BuybackMetric label="Bought & burned" value={snapshot ? number(snapshot.totalTokensBurned) : "—"} />
      <BuybackMetric label="Executions" value={snapshot ? number(snapshot.executionCount) : "—"} />
      <BuybackMetric label="Pending reserve" value={snapshot ? money(snapshot.reserveUsdc, true) : "—"} />
    </div>
    ...
    {error && <div className="px-4 pb-4"><WarningBox>{error}</WarningBox></div>}
  </Panel>;
}
