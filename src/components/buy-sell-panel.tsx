...
function LiveBuySellPanel({ token, poolAddress }: { token: TokenData; poolAddress: Address }) {
  ...
  const [slippageInput, setSlippageInput] = useState("3");
  ...
  return <div id="trade-panel" className="panel scroll-mt-28 rounded-[28px] p-5 shadow-none">
    ...
    <div className="mt-2 grid grid-cols-5 gap-1">{percentageOptions.map((percent) => <button key={percent} type="button" disabled={isPending || !activeBalance} onClick={() => activeBalance !== undefined && setAmount(inputUnits(activeBalance * BigInt(percent) / 100n, inputDecimals))} className="h-9 rounded-full font-mono text-sm text-slate-400 disabled:opacity-35">{percent}%</button>)}</div>
    ...
    <div className="mt-3 grid gap-3 py-2"><div className="flex items-center justify-between"><span className="text-slate-300">Slippage</span><div className="flex gap-1">{slippageOptions.map((value) => <button key={value} type="button" onClick={() => setSlippageInput(String(value))} className={`h-9 rounded-full px-3 font-mono text-sm ${slippage === value ? "bg-cyan/12 text-cyan" : "text-slate-400"}`}>{value}%</button>)}<label className="flex h-9 w-[86px] items-center rounded-full border border-line px-2"><input aria-label="Custom slippage percentage" inputMode="decimal" value={slippageInput} onChange={(event) => /^\d{0,2}(?:\.\d{0,2})?$/.test(event.target.value) && setSlippageInput(event.target.value)} className="min-w-0 flex-1 bg-transparent text-right outline-none" /><span>%</span></label></div></div><div className="flex items-center justify-between"><span className="text-slate-300">Priority</span><div className="flex gap-1">{priorityOptions.map((value) => <button key={value} type="button" onClick={() => setPriority(value)} className={`h-9 rounded-full px-3 text-sm ${priority === value ? "bg-cyan/12 text-cyan" : "text-slate-400"}`}>{value}</button>)}</div></div></div>
